...
import { Menu, X, ShoppingBag, User, BookOpen, Heart, LayoutDashboard, LogOut } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 backdrop-blur-md bg-cream/80 dark:bg-navy/80 border-b border-orange/10 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between h-16">
        <Link to="/" className="flex items-center gap-2">
          <motion.div whileHover={{ rotate: -10, scale: 1.1 }} className="bg-orange text-white p-2 rounded-xl">
            <BookOpen size={20} />
          </motion.div>
          <span className="font-bold text-lg text-navy dark:text-cream">Read on Rent</span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          {/* Admin link only rendered for role === 'admin'.
              Real enforcement is backend adminOnly middleware — this is UX only. */}
          {isAdmin && (
            <NavLink
              to="/admin"
